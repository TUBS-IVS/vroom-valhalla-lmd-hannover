...
%
% FIGURE SOURCES. Every figure below is rendered by the revision scripts into
%   results/revision_2026_08_v6/figures/
% and copied into paper/EWGT_2026_rev1/figures/ by
% scripts/revision/71_sync_paper_figs.py, which md5-verifies each copy. All
% eleven are tracked there, so \graphicspath needs only {figures/} and this
% document builds standalone from the paper folder alone.
% 71_ RENAMES on sync -- the names below are its DESTINATION names:
%   supp_fig4b_mean_days        -> supp_fig4b_mean_delivery_days
%   supp_fig5_grid_heatmap_v2   -> supp_fig5_two_lens_heatmaps
%   supp_fig5b_offdiagonal_v2   -> supp_fig5b_offdiagonal_lens_plan
%   supp_fig6_structural_v2     -> supp_fig6_per_cell_euro
%   supp_fig6b_operator_lens_v2 -> supp_fig6b_operator_lens_hub
%   supp_map_*_v2 / supp_penalty_raumtyp_v2 -> the same stems without _v2
%
...
generated. Over the surrogate's training pool the three terms contribute $72.2$,
$6.0$ and $21.7\%$ of label cost; over the re-routed validation labels, a
different instance mix, they contribute $70.7$, $6.1$ and $23.2\%$.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_fig4b_mean_delivery_days.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_fig5_two_lens_heatmaps.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_fig5b_offdiagonal_lens_plan.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_fig6_per_cell_euro.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_fig6b_operator_lens_hub.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_map_saving_P.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_penalty_raumtyp.pdf}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_map_freq_theta.pdf}
  ...
  adoption; a single area reaches two.}
  \label{fig:supp_map_freq}
\end{figure}

\begin{figure}[h]
  \centering
  \includegraphics[width=\linewidth]{supp_map_wait_theta.pdf}
  ...
  plan. The maximum area value is $0.6$ days, at full adoption.}
...
\begin{figure}[h]
  ...
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{supp_fig_fleet_week_P0.pdf}\\[2mm]
    {\small (a) $P = 0$}
  \end{minipage}\\[3mm]
  \begin{minipage}{\linewidth}
    \centering
    \includegraphics[width=\linewidth]{supp_fig_fleet_week_P025.pdf}\\[2mm]
    {\small (b) $P = 0.25$}
  \end{minipage}
  \caption{Monday--Saturday vehicles per LSP at full adoption ($\theta = 1$), for
  the daily-delivery baseline, the routing-optimal plan of stage~1 --- recomputed
  with the partition-aware fleet counter --- and the operator-polished plan of
  stage~2. Summed depot peaks over the seven providers and their $22$ depots go
  $1{,}239 \rightarrow 1{,}666 \rightarrow 1{,}030$ vehicles at $P = 0$ (a) and
  $1{,}239 \rightarrow 1{,}314 \rightarrow 1{,}026$ at $P = 0.25$ (b): the
  routing-optimal plan \emph{raises} the peak fleet it never pays for, and only
  the operator polish converts the freedom to move a delivery day into a flatter
  week. Vehicle counts are model outputs computed from the HAGRID-based demand
  and the tour rule of Section~2.2 of the main text; they are not carrier data
  and no operator reported them.}
  \label{fig:supp_fleet_week_lsp}
\end{figure}
