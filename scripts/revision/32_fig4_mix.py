...
FROZEN (2026-08 revision): builds the accepted paper Fig. 4; see the banner
...
_frozen_notice.warn(
    "32_fig4_mix.py is FROZEN: it builds the accepted paper Fig. 4 and its "
    "plotting code must not change. Do NOT run it against the 2026-07 "
    "tables and quote the result -- that schema has no pool term and "
    "predates the universal tour rule and the two cost lenses. Run it only "
    "through scripts/revision/74_v2_to_legacy_tables.py --render, which "
    "adapts a v5/v6 grid to the schema this file reads and sets REV_DIR / "
    "REV_RUN_DIR / REV_BASE_TOTAL / REV_BASELINE_CV accordingly. The "
    "two-lens designs live in 70_figs_tables_v2.py as supp_*.",
    UserWarning,
    stacklevel=2,
)
# ---------------------------------------------------------------------------
