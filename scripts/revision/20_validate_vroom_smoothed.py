"""VROOM out-of-sample validation of the SYSTEM-SMOOTHED (Stage-3) schedules.

Routes the chosen *system-smoothed* weekly schedules through VROOM + Valhalla
and compares the ML-predicted per-(provider, PLZ) batched delivery cost
(`dd_cost_system_smoothed`) against the VROOM-actual cost.

Cells validated (paper operating point + a batching sweep):
    (P=0.0,  share=1.0)
    (P=0.25, share=1.0)
    (P=0.5,  share=1.0)
    (P=0.75, share=1.0)
...
dominant, surrogate-predicted component. The hub-bundled express residual is
a separate cost component (also ML-predicted) and is NOT re-routed here,
matching the prior overnight/balanced validation methodology (see Task 10 for
the paper-text scope note).

Reuses the proven primitives from `_stage3_common` (fs_b2c/fs_b2b willingness
split, schedule enumeration, model loader, checkpoint loader) and the
routing core.
...
Output (results/revision_2026_07/validation/):
    tab_vroom_smoothed.csv        per-(cell, provider, plz, day) VROOM solves
...
OUT = C.OUT_DIR / "validation"
OUT.mkdir(parents=True, exist_ok=True)

CHECKPOINT = OUT / "tab_vroom_smoothed.csv"
...
def append_rows(rows: list[dict]) -> None:
    """Append checkpoint rows; retry on transient Windows file locks.

    The 2026-07-16 overnight run died with PermissionError [Errno 13] on this
    append (morning backup/AV/sync briefly locking the CSV). Losing hours of
    resumable progress to a transient lock is unacceptable, so retry with
    ...
    for attempt in range(60):
        try:
            df.to_csv(CHECKPOINT, mode="a", header=not CHECKPOINT.exists(),
                      index=False)
            return
        except PermissionError as e:  # transient lock — wait and retry
            last_err = e
            if attempt == 0:
                log(f"  WARNING: checkpoint locked ({e}); retrying up to 5 min")
            time.sleep(5)
    raise last_err
...
def solve_plz_batched(prov, plz, sched_days, pdata, pd_, hub_row, prefix,
    ...
    """Route every delivery day of one (provider, PLZ) system-smoothed schedule."""
    rows = []
    ...
    for dd in sched_days:
        ...
        vehicles, _ = build_vroom_vehicles(
            ...
            seed_key=f"sm_P{penalty}_s{share:.1f}_{prov}_{plz}_d{dd}", n_jobs=len(jobs))
        result = solve_single_plz(
            ...
            cache_tag=f"smval_p{penalty}_s{share:.1f}_{prov}")
        rows.append({"penalty": penalty, "share_willing": share,
                     "provider": prov, "plz": plz, "day": dd, "mode": "batched",
...
def run():
    ...
    log("=" * 70)
    log("VROOM validation of SYSTEM-SMOOTHED schedules" + (" [SMOKE]" if smoke else ""))
    ...
    for penalty, share in VALIDATION_CELLS:
        ...
        for prov in provs:
            ...
            for j, (_, r) in enumerate(prov_sub.iterrows()):
                ...
                if hub_row_df.empty:
                    continue
                sched_days = sched_by_idx[int(r.schedule_idx_system_smoothed)]
                ...
                # attach ML predicted system-smoothed delivery cost for direct comparison
                for row in rows:
                    row["ml_dd_cost_system_smoothed_eur"] = float(r.dd_cost_system_smoothed)
                    row["schedule_size_system_smoothed"] = int(r.schedule_size_system_smoothed)
                append_rows(rows)
                done.add(key)
                n_done_cell += 1
                if (j + 1) % 20 == 0:
...
def assemble():
    """Weekly-aggregate VROOM truth vs ML dd_cost_system_smoothed; MAPE + scatter."""
    if not CHECKPOINT.exists():
    ...
    wk = (df.groupby(["penalty", "share_willing", "provider", "plz"], as_index=False)
            .agg(vroom_cost_eur=("vroom_cost_eur", "sum"),
                 ml_dd_cost_eur=("ml_dd_cost_system_smoothed_eur", "first"),
                 n_routes=("vroom_n_routes", "sum"),
                 n_parcels=("vroom_n_parcels", "sum")))
    wk = wk[wk.vroom_cost_eur > 0].copy()
    wk.to_csv(OUT / "tab_ml_vs_vroom_smoothed.csv", index=False)
    ...
    diag_df.to_csv(OUT / "tab_diagnostics_smoothed.csv", index=False)
    log("\nDiagnostics (ML dd_cost_system_smoothed vs VROOM actual):")
    ...
    for ax, (P, s) in zip(axes[0], cells):
        ...
        ax.set_ylabel("ML predicted (dd_cost_system_smoothed) [€]")
        ax.set_title(f"P={P}, share={s}\nMAPE={d['MAPE_pct']:.2f}%  "
                     f"bias={d['bias_pct']:+.2f}%  R²={d['R2']:.3f}  n={d['n']}")
        ax.grid(alpha=0.3, which="both")
    fig.suptitle("Surrogate vs VROOM on system-smoothed optimized schedules", y=1.02)
    fig.tight_layout()
    fig.savefig(OUT / "fig_vroom_vs_ml_smoothed.png")
    fig.savefig(OUT / "fig_vroom_vs_ml_smoothed.pdf")
    plt.close(fig)
    log(f"  wrote fig_vroom_vs_ml_smoothed + tables to {OUT}")
