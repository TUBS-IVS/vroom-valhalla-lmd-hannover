...
Recomputes the full 312×39 cost matrix with the new median-calibrated model,
then derives:
  10_sensitivity/  fig_S1 cost-per-parcel vs schedule size (break-even)
                   fig_S2 marginal cost curve (1d→6d incremental)
                   fig_S3 break-even map (demand × area → optimal schedule size)
...
def lgb_feature_importance():
    ...
    plt.close(fig)
    print("  ✓ M5: lgb_feature_importance (new model)")
...
def sensitivity():
    ...
    print(f"  recomputing cost matrix: {len(pp_list)} cells × {len(schedules)} schedules (new model)")
    ...
    plt.close(fig)
    print("  ✓ S2: marginal_cost")

    # S3: break-even map — optimal (lowest-cost) schedule size in (demand × area)
    ...
    ax.set_title("Cost-minimal schedule size in (demand × area) space\n"
...
def copy_input_descriptive():
    ...
    print(f"  ✓ input-descriptive raumtyp maps: {n} files (data-intrinsic, model-independent)")
...
"""
    ...
    print("  ✓ 07_validation/STATUS.md (documents pending VROOM)")
