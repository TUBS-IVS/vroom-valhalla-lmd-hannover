...
One panel per LSP: median saving over (share willing × hub distance), thick line =
...
OUT = ROOT / "results" / "paper_final_2026_05_28" / "09_region_analysis"
rcParams.update({
...
def main():
    ...
    fig.suptitle("Per-LSP iso-saving map — share willing × hub distance "
    ...
    plt.close(fig)
    print("  ✓ fig_BE9_isosaving_per_provider")

    # ── Robust alternative: annotated heatmap grid (hub-dist band × share) ──
    ...
    fig.suptitle("Per-LSP median saving — hub-distance band × share willing (bundled)",
                 fontsize=14, y=0.99)
    fig.tight_layout(rect=[0, 0, 0.97, 0.97])
    fig.savefig(OUT / "fig_BE9b_saving_heatmap_per_provider.png")
    fig.savefig(OUT / "fig_BE9b_saving_heatmap_per_provider.pdf")
    plt.close(fig)
    print("  ✓ fig_BE9b_saving_heatmap_per_provider")
