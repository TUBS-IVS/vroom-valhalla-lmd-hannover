...
Generates all 8 sections of plots fresh from results/overnight_2026_05_27_balanced/
...
Sections:
  ...
  05_optimization/       Pareto, P×share, schedule mix, sweet-spot — ALL FRESH
...
def fig_baseline(agg, sched):
    ...
    plt.close(fig)
    print("  ✓ B1: baseline_per_provider")
...
def fig_training():
    ...
    ax.set_title(f"Training pool: {len(train):,} samples × 7 LSPs")
    ...
    plt.close(fig)
    print("  ✓ T1: training_distribution")
...
def fig_optimization(agg, sched, baseline):
    ...
    plt.close(fig)
    print("  ✓ SM2: provider_schedule_share100")
...
def fig_interpretation(sched):
    ...
    if fdf.empty:
        print("  ✗ No data for decision trees")
        return
    ...
    print(f"  ✓ DT1: schedule_choice (acc {acc:.1%})")
    ...
    print(f"  ✓ DT2: saving_pct (R²={r2:.3f})")
...
def write_readme(agg, baseline):
    ...
    txt = [
        ...
        "3. **Optimization**: per-(P, share) cell, 312 (provider, PLZ) cells × 39 schedules, argmin with "
