...
Adds:
  ...
  05_optimization/ fig_O3 schedule landscape (demand × area → chosen schedule)
                   fig_O4 sensitivity: cost-per-parcel by schedule size
  09_region_analysis/  fig_R1 saving by raumtyp_3
                       fig_R2 provider × raumtyp heatmap
                       fig_R3 PLZ choropleth-style scatter (hub_dist × area → saving)
...
def fig_region(df):
    ...
    plt.close(fig)
    print("  ✓ R1: saving_by_raumtyp")

    # R2: provider × raumtyp heatmap
    ...
    ax.set_title("Saving % per LSP × region type (P=0.4, share=100%)")
    fig.tight_layout()
    fig.savefig(d / "fig_R2_provider_x_raumtyp.png"); fig.savefig(d / "fig_R2_provider_x_raumtyp.pdf")
    plt.close(fig)
    print("  ✓ R2: provider_x_raumtyp")

    # R3: hub_dist × area → saving scatter (the "theory test")
    ...
    ax.set_title("Where batching pays off: hub-distance × area → saving%\n"
    ...
    plt.close(fig)
    print("  ✓ R3: hubdist_area_saving")

    # R4: hub-dist bucketed × raumtyp crosstab
...
def fig_schedule_landscape(df):
    """05: which schedule wins in (demand × area) space."""
    ...
    ax.set_xscale("log")
    ax.set_title("Schedule landscape — chosen frequency in (demand × area) space\n"
