...
# Clean run uses Daganzo-LGB-Hybrid trained on v2-augmented full pool.
MODEL_PATH  = RUN_DIR / "daganzo_hybrid_v2aug.pkl"
OUT_DIR     = ROOT / "results" / "final_optimization_clean"
...
def main():
    ...
    # ── inject Daganzo-LGB-Hybrid as ML predictor ─────────────────────
    log.info("► Stage 4 · loading Daganzo-LGB-Hybrid surrogate (no MLP retrain)")
    if not MODEL_PATH.exists():
        raise FileNotFoundError(
            f"Daganzo-Hybrid model not found at {MODEL_PATH}. "
            "Run scripts/train_daganzo_hybrid.py first."
        )
    import pickle as _pkl
    # Pre-import the module that defines _LGBIdentityWrap so pickle.load
    # can resolve it (pickle stores class refs by module + name).
    sys.path.insert(0, str(ROOT / "scripts"))
    import train_daganzo_hybrid as _tdh
    from train_daganzo_hybrid import DaganzoLGBHybrid, _LGBIdentityWrap  # noqa: F401
    # Bind to __main__ namespace too — _LGBIdentityWrap was pickled with
    # module="__main__" from inside the trainer script.
    import __main__
    ...
    if kind == "DaganzoLGBHybrid":
        ml_predictor = DaganzoLGBHybrid(
            model=_data["model"], combo_cols=_data["combo_cols"],
            alpha=_data["alpha"],
        )
        log.info("✓ Daganzo-Hybrid loaded (%d combo features, alpha=%.3f)",
                  len(ml_predictor.combo_cols), ml_predictor.alpha)
    else:
        ml_predictor = LGBLogTSurrogate.load(MODEL_PATH)
        log.info("✓ LGB-logT loaded (%d combo features)",
                  len(ml_predictor.combo_cols))
    ...
    # ── EARLY EXIT for ML-only optimization run ────────────────────────
    # User wants to inspect chosen schedules + ML-predicted costs WITHOUT
    # the 12h VROOM re-routing pass. Persist the CD-chosen schedules and
    # ML cost predictions, then stop.
    import os
    if os.environ.get("SKIP_VROOM_VALIDATION", "0") == "1":
        log.info("► EARLY EXIT (SKIP_VROOM_VALIDATION=1) — saving ML-only schedules")
        import pickle as _pkl
        out = OUT_DIR / "ml_only_schedules.pkl"
        with open(out, "wb") as f:
            _pkl.dump({
                "scenario_schedules_by_provider": scenario_schedules_by_provider,
                "ml_optimization_data": ml_opt,
                "optimization_data": odata,
                "model_kind": kind,
            }, f)
        log.info("✓ Saved ML-only optimization to %s", out)
        # Also write a per-(provider, plz) schedule summary CSV
        rows = []
        for provider in cfg.providers:
            for sc_name in [SC_SA_ML_BATCH, SC_FIXED_BATCH]:
                schedules_dict = scenario_schedules_by_provider[provider].get(sc_name, {})
                for plz, sched in schedules_dict.items():
                    rows.append({
                        "provider": provider, "plz": plz, "scenario": sc_name,
                        "schedule_size": len(sched),
                        "schedule_days": sorted(list(sched)),
                    })
        import pandas as _pd
        _pd.DataFrame(rows).to_csv(OUT_DIR / "ml_only_schedules.csv", index=False)
        log.info("✓ Saved %d rows to %s/ml_only_schedules.csv", len(rows), OUT_DIR)
        return
