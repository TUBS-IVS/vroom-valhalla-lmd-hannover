...
_deprecation_warnings.warn(
    "paper_final_maps.py is a STALE entry point: it recomputes totals WITHOUT the pool "
...
def fig_maps(chosen):
    ...
    plt.close(fig)
    print("  [OK] MAP1: freq_choropleth_per_P")
    ...
    plt.close(fig)
    print("  [OK] MAP2: saving_choropleth_per_P")
...
def fig_freq_mix(chosen):
    ...
    plt.close(fig)
    print("  [OK] O6: freq_mix_vs_share_per_P")
...
def fig_value_of_optimization(chosen):
    ...
    plt.close(fig)
    print("  [OK] O7: value_of_optimization")
