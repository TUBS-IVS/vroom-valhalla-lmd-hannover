...
Adds:
  ...
  10_sensitivity_2d/           — 2D batch x penalty heatmap
...
_deprecation_warnings.warn(
    "paper_final_extras_v2.py is a STALE entry point: it recomputes totals WITHOUT the pool "
...
def main():
    ...
    for stem in cost_decomp_files:
        for ext in (".png", ".pdf"):
            ...
            if src.exists():
                ...
                n_cost += 1
    print(f"  [OK] 16_cost_decomposition_full: {n_cost} files")
    ...
    for stem in spatial_files:
        for ext in (".png", ".pdf"):
            ...
            if src.exists():
                ...
                n_spat += 1
    print(f"  [OK] 01_input_data/additional: {n_spat} files")
    ...
    for stem in extra_model:
        for ext in (".png", ".pdf"):
            ...
            if src.exists():
                ...
                n_em += 1
    print(f"  [OK] 04_model/additional: {n_em} files")
    ...
    extra_index = [
        ...
        "- R1: saving by raumtyp, R2: provider x raumtyp, R3: MAPE by raumtyp, R4: choropleth",
        "",
        "### 12_ml_quality_per_cluster/",
        "- MLA1-6: per-raumtyp boxplot, provider x raumtyp heatmaps, cluster bias choropleth,",
    ...
    if "## Full sub-folder index" not in base:
        ...
        print(f"  [OK] README.md updated with full index")
