...
LGB_PATH    = RUN_DIR / "production_lgb_logT_v2.pkl"
OUT_DIR     = ROOT / "results" / "final_optimization_v2"
...
def main():
    ...
    # ── invalidate v1 optimization + scenario-routing checkpoints so v2
    #    model produces a fresh CD optimum + scenario VROOM solves ─────
    ckp_dir = ROOT / "results" / "checkpoints"
    for ckp_name in ["08_sa_ml_optimization.pkl", "06_scenario_routing.pkl"]:
        ckp_path = ckp_dir / ckp_name
        if ckp_path.exists():
            archive_dir = ckp_dir / "archive"
            archive_dir.mkdir(exist_ok=True)
            new_path = archive_dir / f"v1_{ckp_name}"
            ckp_path.rename(new_path)
            log.info("Archived %s -> %s", ckp_name, new_path)
